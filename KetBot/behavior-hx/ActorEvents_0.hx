...
class ActorEvents_0 extends ActorScript
{
	...
	override public function init()
	{
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if((Math.abs((actor.getY() - Engine.engine.getGameAttribute("Y of Andy"))) <= Engine.engine.getGameAttribute("areaOfEffectY")))
				{
					if((Math.abs((actor.getX() - Engine.engine.getGameAttribute("x of Andy"))) <= Engine.engine.getGameAttribute("areaOfEffect")))
					{
						runLater(1000 * .3, function(timeTask:TimedTask):Void
						{
							...
							{
								if(((Math.abs((actor.getX() - Engine.engine.getGameAttribute("x of Andy"))) <= Engine.engine.getGameAttribute("areaOfEffect")) && (Math.abs((actor.getY() - Engine.engine.getGameAttribute("Y of Andy"))) <= (Engine.engine.getGameAttribute("areaOfEffectY") - 1))))
								{
