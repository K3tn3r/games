...
class SceneEvents_3 extends SceneScript
{
	...
	override public function init()
	{
		...
		Engine.engine.setGameAttribute("Jump3", true);
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				...
				{
					Engine.engine.setGameAttribute("Spawn X", 2780);
					Engine.engine.setGameAttribute("Spawn Y", 230);
				}
			}
		});
		...
		addActorEntersRegionListener(getRegion(0), function(a:Actor, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && sameAsAny(getActorType(2),a.getType(),a.getGroup()))
			{
				switchScene(GameModel.get().scenes.get(4).getID(), createFadeOut(.5, Utils.getColorRGB(0,0,0)), createFadeIn(.5, Utils.getColorRGB(0,0,0)));
			}
		});
