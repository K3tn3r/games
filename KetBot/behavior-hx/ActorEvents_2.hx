...
class ActorEvents_2 extends ActorScript
{
	...
	/* ========================= Custom Event ========================= */
	public function _customEvent_killBloock1():Void
	{
		_score = asNumber((_score + 1));
		propertyChanged("_score", _score);
		Engine.engine.setGameAttribute("score", _score);
	...
	/* ========================= Custom Event ========================= */
	public function _customEvent_killBloock2():Void
	{
		_score = asNumber((_score + 1));
		propertyChanged("_score", _score);
		Engine.engine.setGameAttribute("score", _score);
	...
	/* ========================= Custom Event ========================= */
	public function _customEvent_killBoofer():Void
	{
		_score = asNumber((_score + 2));
		propertyChanged("_score", _score);
		Engine.engine.setGameAttribute("score", _score);
	}
	...
	public function _customEvent_dead():Void
	{
		if(_isalive)
		{
			...
			{
				reloadCurrentScene(createFadeOut(.5, Utils.getColorRGB(0,0,0)), createCircleIn(.5, Utils.getColorRGB(0,0,0)));
			}, actor);
		}
	}
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if(_isalive)
				{
					...
					if(((isKeyDown("up") && isKeyDown("left")) && _inair))
					{
						_isJumping = true;
						propertyChanged("_isJumping", _isJumping);
						actor.setAnimation("" + "Jump left");
