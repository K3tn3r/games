...
class SceneEvents_0 extends SceneScript
{
	...
	override public function init()
	{
		...
		{
			Engine.engine.setGameAttribute("Spawn X", 315);
			Engine.engine.setGameAttribute("Spawn Y", 200);
		}
		...
		addWhenDrawingListener(null, function(g:G, x:Float, y:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				...
				g.drawString("" + Engine.engine.getGameAttribute("checkpiont"), 32, 40);
				g.drawString("" + Engine.engine.getGameAttribute("Spawn X"), 100, 100);
				g.drawString("" + Engine.engine.getGameAttribute("Spawn Y"), 100, 120);
				g.drawString("" + Engine.engine.getGameAttribute("X of Camera"), 200, 120);
			}
		});
		
		/* ======================== Actor of Type ========================= */
		addActorEntersRegionListener(getRegion(5), function(a:Actor, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && sameAsAny(getActorType(2),a.getType(),a.getGroup()))
			{
				Engine.engine.setGameAttribute("checkpiont", true);
				Engine.engine.setGameAttribute("Spawn X", 4010);
				Engine.engine.setGameAttribute("Spawn Y", 250);
			}
		});
		...
		addWhenDrawingListener(null, function(g:G, x:Float, y:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if(_draw)
				{
					g.drawString("" + "Press \"R\" to retry/restart", 160, 240);
					g.drawString("" + "Press \"P\" to pause/unpause", 160, 260);
				}
			}
		});
