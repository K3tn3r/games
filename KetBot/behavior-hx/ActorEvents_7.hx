...
class ActorEvents_7 extends ActorScript
{
	...
	override public function init()
	{
		...
		/* ========================= Type & Type ========================== */
		addSceneCollisionListener(getActorType(2).ID, getActorType(7).ID, function(event:Collision, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if((event.otherFromTop && _isalive))
				{
					actor.shout("_customEvent_" + "death");
					event.thisActor.shout("_customEvent_" + "killBlob");
				}
				else if((!(event.otherFromTop) && _isalive))
				{
					event.thisActor.shout("_customEvent_" + "dead");
					_left = !(_left);
					propertyChanged("_left", _left);
					_turntimer = asNumber(3);
					propertyChanged("_turntimer", _turntimer);
